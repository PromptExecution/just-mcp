use just_mcp_lib::environment::{
    McpEnvironment, EnvironmentSource, load_mcp_environment, 
    validate_mcp_environment, get_environment_info, MCP_ENVIRONMENT_VARIABLES
};
...
fn test_mcp_environment_snapshot_workflow() {
    ...
    let expanded = env.expand_variables("Server: ${MCP_SERVER_NAME} at ${MCP_LOG_LEVEL} level").unwrap();
...
fn test_mcp_environment_variable_expansion_edge_cases() {
    ...
    let result = env.expand_variables("Config: ${MCP_PREFIX}-${MCP_SUFFIX}-config").unwrap();
