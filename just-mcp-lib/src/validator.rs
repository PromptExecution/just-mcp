...
pub fn format_signature_help(help: &SignatureHelp) -> String {
    ...
    let param_strings: Vec<String> = help.parameters
        .iter()
        .map(|param| {
            if param.required {
                param.name.clone()
            } else {
                format!("{}={}", param.name, param.default_value.as_deref().unwrap_or(""))
            }
        })
        .collect();
...
mod tests {
    ...
    fn test_validate_arguments_missing_required() {
        ...
        assert!(result.errors[0].message.contains("Missing required parameter"));
    }
    ...
    fn test_get_signature_help() {
        let params = vec![
            Parameter { name: "env".to_string(), default_value: None },
            Parameter { name: "target".to_string(), default_value: Some("prod".to_string()) },
            Parameter { name: "verbose".to_string(), default_value: Some("false".to_string()) },
        ];
        let recipe = create_test_recipe("deploy", params);
