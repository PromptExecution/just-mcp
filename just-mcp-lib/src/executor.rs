...
pub enum ExecutionError {
    ...
    InvalidArguments { recipe_name: String, message: String },
    
    #[snafu(display("Dependency '{}' failed for recipe '{}': {}", dependency, recipe_name, source))]
    DependencyFailed { 
        recipe_name: String, 
        dependency: String, 
...
pub fn execute_recipe(
    ...
    for dep in &recipe.dependencies {
        let dep_result = execute_recipe(justfile, dep, &[], working_dir)
...
mod tests {
    ...
    fn create_test_recipe(name: &str, params: Vec<Parameter>, body: &str, deps: Vec<&str>) -> Recipe {
        Recipe {
            name: name.to_string(),
            parameters: params,
            documentation: None,
    ...
    fn test_validate_arguments_success() {
        let params = vec![
            ...
            Parameter { name: "target".to_string(), default_value: Some("prod".to_string()) },
        ];
    ...
    fn test_substitute_parameters() {
        ...
        assert_eq!(result, "echo 'Deploying staging on port 8080 version 1.0.0'");
    }
